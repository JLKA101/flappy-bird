...
while True:
    ...
    birdGroup.update()
    scorecard = font.render(f"Score: {score}", True, "white")
    screen.blit(scorecard, (432, 50))
    #checking the score
    if len(pipeGroup) > 0:
        if birdGroup.sprites()[0].rect.left > pipeGroup.sprites()[0].rect.left and birdGroup.sprites()[0].rect.right < pipeGroup.sprites()[0].rect.right and pass_pipe == False:
            pass_pipe = True
        if pass_pipe == True:
